...
async def on_ready():
  ...
  print(f"Connecté à : {len(client.guilds)} guilds")
  print(f"Connecté à : {len(client.users)} users")
  print("Crée par @monkey_d_.")
  try:
...
async def on_command_completion(context: Context) -> None:
  ...
  if context.guild is not None:
    try:
      ...
      embed.set_author(
        name=f"Exécuté {executed_command} Command par : {context.author}",
        icon_url=f"{context.author.avatar}")
      embed.set_thumbnail(url=f"{context.author.avatar}")
      embed.add_field(name="Nom de la Command :",
                      value=f"{executed_command}",
                      inline=False)
      embed.add_field(
        name="Command Exécuté par :",
        value=
